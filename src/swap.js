...
async function pumpFunBuy(transactionMode, payerPrivateKey, mintStr, solIn, priorityFeeInSol = 0, slippageDecimal = 0.25, walletuser = "7KMACE") {
    try {
        ...
        const mcap = coinData.usd_market_cap;
        if (mcap > constants_1.MCAPMAX) {
            console.log(`${walletuser} High Mcap ${mcap}`);
            return;
        }
        const time = Date.now();
        console.log(`Buy start ${walletuser}`);
        console.time(`Execution Time ${walletuser} ${time}`);
        ...
        const tokenOut = Math.floor((solInLamports * coinData["virtual_token_reserves"]) /
        ...
        if (transactionMode == types_1.TransactionMode.Execution) {
            ...
            console.log(`Buy transaction confirmed username: ${walletuser}`);
            console.timeEnd(`Execution Time ${walletuser} ${time}`);
...
async function pumpFunSell(transactionMode, payerPrivateKey, mintStr, tokenBalance, priorityFeeInSol = 0, slippageDecimal = 0.25, walletuser = "7KMACE") {
    try {
        const connection = new web3_js_1.Connection(constants_1.RPC, "confirmed");
        const coinData = await (0, api_1.getCoinData)(mintStr);
        if (!coinData) {
        ...
        const time = Date.now();
        console.log(`Sell start ${walletuser}`);
        console.time(`Execution Time ${walletuser} ${time}`);
        const payer = await (0, utils_1.getKeyPairFromPrivateKey)(payerPrivateKey);
        const owner = payer.publicKey;
        const mint = new web3_js_1.PublicKey(mintStr);
        ...
        if (!tokenAccountInfo) {
            ...
            tokenBalance = parseInt(amount);
            const power = 10 ** 6;
            console.log(`${walletuser} Balance: ${tokenBalance / power}`);
        }
        const minSolOutput = Math.floor((tokenBalance *
            (1 - slippageDecimal) *
            coinData["virtual_sol_reserves"]) /
        ...
        if (transactionMode == types_1.TransactionMode.Execution) {
            const signature = await (0, utils_1.sendAndConfirmTransactionWrapper)(connection, transaction, [payer]);
            console.log(`Sell transaction confirmed user: ${walletuser}`);
            console.timeEnd(`Execution Time ${walletuser} ${time}`);
            console.log(`https://solscan.io/tx/${signature}`);
            console.log("");
            console.log("");
